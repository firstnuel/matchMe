...
func (_q *MessageQuery) QueryConnection() *ConnectionQuery {
...
func (_q *MessageQuery) QuerySender() *UserQuery {
...
func (_q *MessageQuery) QueryReceiver() *UserQuery {
	query := (&UserClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *MessageQuery) WithConnection(opts ...func(*ConnectionQuery)) *MessageQuery {
	query := (&ConnectionClient{config: _q.config}).Query()
...
func (_q *MessageQuery) WithSender(opts ...func(*UserQuery)) *MessageQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MessageQuery) WithReceiver(opts ...func(*UserQuery)) *MessageQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(message.FieldConnectionID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Message.Query().
//		Select(message.FieldConnectionID).
//		Scan(ctx, &v)
...
func (_q *MessageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Message, error) {
	...
	if query := _q.withConnection; query != nil {
		if err := _q.loadConnection(ctx, query, nodes, nil,
			func(n *Message, e *Connection) { n.Edges.Connection = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withReceiver; query != nil {
		if err := _q.loadReceiver(ctx, query, nodes, nil,
			func(n *Message, e *User) { n.Edges.Receiver = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *MessageQuery) loadConnection(ctx context.Context, query *ConnectionQuery, nodes []*Message, init func(*Message), assign func(*Message, *Connection)) error {
...
func (_q *MessageQuery) loadSender(ctx context.Context, query *UserQuery, nodes []*Message, init func(*Message), assign func(*Message, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Message)
	for i := range nodes {
		fk := nodes[i].SenderID
...
func (_q *MessageQuery) loadReceiver(ctx context.Context, query *UserQuery, nodes []*Message, init func(*Message), assign func(*Message, *User)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Message)
	for i := range nodes {
...
func (_q *MessageQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withConnection != nil {
			_spec.Node.AddColumnOnce(message.FieldConnectionID)
		}
		if _q.withSender != nil {
			_spec.Node.AddColumnOnce(message.FieldSenderID)
		}
		if _q.withReceiver != nil {
			_spec.Node.AddColumnOnce(message.FieldReceiverID)
		}
	}
